...
def main(args):
    ...
    input_image_path = "infrared_image_dataset"
    ...
    feat1 = np.load(
        os.path.join(image_features_path, "features")
        + str(count)
...
def parseArg():
    ...
    parser.add_argument(
        "--model_file",
        type=str,
        default="code/models/d2_tf_no_phototourism.pth",
        help="path to the full model",
    )

    parser.add_argument(
        "--max_edge", type=int, default=1200, help="maximum image size at network input"
    )
    parser.add_argument(
        "--max_sum_edges",
        type=int,
    ...
    parser.add_argument(
        "--output_extension", type=str, default=".npy", help="extension for the output"
    )
    parser.add_argument(
